...
import { docker, dockerIntro } from '../data';
import SearchBar from '../components/SearchBar';

export default function DockerScreen() {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredData = docker.filter(item => {
  ...
  const renderHeader = () => (
    <View style={styles.introContainer}>
      <Text style={styles.introTitle}>{dockerIntro.title}</Text>
      <Text style={styles.introDesc}>{dockerIntro.desc}</Text>
      <View style={styles.featuresContainer}>
        {dockerIntro.keyFeatures.map((feature, index) => (
