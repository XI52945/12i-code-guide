...
import { sql, sqlIntro } from '../data';
import SearchBar from '../components/SearchBar';

export default function SqlScreen() {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredData = sql.filter(item => {
  ...
  const renderHeader = () => (
    <View style={styles.introContainer}>
      <Text style={styles.introTitle}>{sqlIntro.title}</Text>
      <Text style={styles.introDesc}>{sqlIntro.desc}</Text>
      <View style={styles.featuresContainer}>
        {sqlIntro.keyFeatures.map((feature, index) => (
