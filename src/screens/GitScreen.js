...
import { git, gitIntro } from '../data';
import SearchBar from '../components/SearchBar';

export default function GitScreen() {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredData = git.filter(item => {
  ...
  const renderHeader = () => (
    <View style={styles.introContainer}>
      <Text style={styles.introTitle}>{gitIntro.title}</Text>
      <Text style={styles.introDesc}>{gitIntro.desc}</Text>
      <View style={styles.featuresContainer}>
        {gitIntro.keyFeatures.map((feature, index) => (
